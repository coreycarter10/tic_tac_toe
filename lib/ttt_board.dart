class TTTBoard {
  static const List<_WinPattern> winPatterns = [
    _WinPattern(0, 1, 2),
    _WinPattern(3, 4, 5),
    _WinPattern(6, 7, 8),
    _WinPattern(0, 3, 6),
  ...
  CellType getWinner() {
    ...
    if (_moveCount >= 5) {
      for (_WinPattern pattern in winPatterns) {
        if (_checkPattern(pattern)) return _grid[pattern.cell1];
      }
    }
    return null;
  }
  ...
  bool get isFull => _moveCount >= _grid.length;
...
String cellTypeToString(CellType type, int index) {
  switch (type) {
